...
rawdata = np.transpose(np.delete(np.genfromtxt('VP16longdata.csv',delimiter=','),0,0))
...
def func(t,d2,k2,k3,k7):
    ...
    for i in range(len(lightdata[:,0])):
        ...
        def odes(z,t,d2,k2,k3,k7):
            ...
            dPudt = d1*Pb - k1*I(t)*Pu + k7*I(t)**n/(Kd**n+I(t)**n)*Pa
            dPbdt = k1*I(t)*Pu - k2*Pb - d1*Pb + d2*Pa
            dPadt = k2*Pb - d2*Pa - k7*I(t)**n/(Kd**n+I(t)**n)*Pa
