...
rawdata = np.transpose(np.delete(np.genfromtxt('rtt102longdata_100.csv',delimiter=','),0,0))
newdata = list(rawdata.flatten())
#new = newdata[0:13920:240]
new = newdata[0:12240:240]
lightdata = np.transpose(np.delete(np.genfromtxt('longlight_100.csv', delimiter=','),0,0))
...
def func(t,d2,k2,k3,k7):
    ...
    for i in range(1):
        ...
        def odes(z,t,d2,k2,k3,k7):
            ...
            dPudt = d1*Pb - k1*I(t)**n/(Kd**n+I(t)**n)*Pu + k7*I(t)*Pa
            dPbdt = k1*I(t)**n/(Kd**n+I(t)**n)*Pu - k2*Pb - d1*Pb + d2*Pa
            dPadt = k2*Pb - d2*Pa - k7*I(t)*Pa
