...
rawdata = np.transpose(np.delete(np.genfromtxt('chz1long_ononly.csv',delimiter=','),0,0))
newdata = list(rawdata.flatten())
#new = newdata[0:24480:240]
lightdata = np.transpose(np.delete(np.genfromtxt('longlight_ononly.csv', delimiter=','),0,0))

def sqres(ptuple):
    return np.sum((np.asarray(newdata)-func(t,*ptuple))**2)

#def func(t,d1,k1,Kd,n,d2,k2,k3):
def func(t,d2,k2,k3):
    ...
    for i in range(len(lightdata[:,0])):
        ...
        #solver = odeint(odes,inivalues,t,args = (d1,k1,Kd,n,d2,k2,k3),hmax=1)
        solver = odeint(odes,inivalues,t,args = (d2,k2,k3),hmax=0.1)

        mCherryout = solver[:,6]
        #mCherry = mCherryout[0:12240:240]
        arrayvalues = np.hstack((arrayvalues,mCherryout))  
